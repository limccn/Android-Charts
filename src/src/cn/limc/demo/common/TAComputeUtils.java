...
public class TAComputeUtils {
    ...
    public static List<IStickEntity> computeMACDData(List<OHLCVData> arrayData,int macdS, int macdL, int macdM) {
    	...
    	for (int i = 0; i < inReal.length; i++) {
    		inReal[i] = Double.parseDouble(arrayData.get(i).getClose());
		}
	...
	public static List<List<DateValueEntity>> computeKDJData(List<OHLCVData> arrayData, int kdjN){
    	...
    	for (int i = 0; i < inHigh.length; i++) {
    		inHigh[i] = Double.parseDouble(arrayData.get(i).getHigh());
		}
    	
    	double[] inLow = new double[arrayData.size()];
    	for (int i = 0; i < inLow.length; i++) {
    		inLow[i] = Double.parseDouble(arrayData.get(i).getLow());
		}
    	
    	double[] inClose = new double[arrayData.size()];
    	for (int i = 0; i < inClose.length; i++) {
    		inClose[i] = Double.parseDouble(arrayData.get(i).getClose());
		}
	...
	public static ListChartData<IStickEntity> convertCandleStickData(List<OHLCVData> ohlcData){
        ...
        for (OHLCVData data : ohlcData) {
			try {
				candleStickData.add(new OHLCEntity(Double.parseDouble(data.getOpen()), Double.parseDouble(data.getHigh()), Double.parseDouble(data.getLow()),Double.parseDouble(data.getClose()), Integer.parseInt(dt.format(df.parse(data.getDate())))));
			} catch (NumberFormatException e) {
				e.printStackTrace();
				continue;
			} catch (ParseException e) {
	...
	public static ListChartData<IStickEntity> convertVOLData(List<OHLCVData> ohlcData){
        ...
        for (OHLCVData data : ohlcData) {
			try {
				if(Double.parseDouble(data.getOpen()) - Double.parseDouble(data.getClose()) > 0){
					stick.add(new ColoredStickEntity(Double.parseDouble(data.getVol()), 0, Integer.parseInt(dt.format(df.parse(data.getDate()))), Color.GREEN));
				}else if (Double.parseDouble(data.getOpen()) - Double.parseDouble(data.getClose()) < 0) {
					stick.add(new ColoredStickEntity(Double.parseDouble(data.getVol()), 0, Integer.parseInt(dt.format(df.parse(data.getDate()))), Color.RED));
				}else {
					stick.add(new ColoredStickEntity(Double.parseDouble(data.getVol()), 0, Integer.parseInt(dt.format(df.parse(data.getDate()))), Color.LTGRAY));
				}
			} catch (NumberFormatException e) {
				e.printStackTrace();
				continue;
